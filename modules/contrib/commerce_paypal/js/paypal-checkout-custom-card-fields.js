(($, Drupal, settings) => {
  Drupal.paypalCheckout = {
    ...
    renderForm(context) {
      ...
      if (paypal.HostedFields.isEligible() !== true) {
        const message = Drupal.t(
          'PayPal has indicated that the current site is not eligible to show the credit card form. Please review your configuration and try again.',
        );
      ...
      const $data = Drupal.paypalCheckout.extractBillingInfo($form);
      $data.flow = 'custom_card_fields';
      paypal.HostedFields.render({
        createOrder: () => {
          return Drupal.paypalCheckout
            .makeCall(Drupal.paypalCheckout.onCreateUrl, {
              type: 'POST',
        ...
        $form.on('submit', (event) => {
          ...
          const formValid = Object.keys(state.fields).every((key) => {
            const { isValid } = state.fields[key];
            if (!isValid) {
          ...
          if (!formValid) {
            ...
            $messagesContainer.html(
              Drupal.theme('commercePaypalError', message),
            );
    ...
    extractBillingInfo($form) {
      ...
      const $profileCopyCheckbox = $(
        ':input[name="payment_information[add_payment_method][billing_information][copy_fields][enable]"]',
        $form,
      );
      if ($profileCopyCheckbox.length && $profileCopyCheckbox.is(':checked')) {
        const shippingInfo = this.extractShippingInfo($form);
      ...
      $form
        .find(
          ':input[name^="payment_information[add_payment_method][billing_information][address][0][address]"]',
        )
        .each((index, element) => {
          ...
          billingInfo.address[name.substring(0, name.length - 1)] =
            $input.val();
        });

      // Fallback to the entered address, if the address fields are present.
      const $addressSelector = $(
        'select[name="payment_information[add_payment_method][billing_information][select_address]"]',
        $form,
      );
      if (
        $.isEmptyObject(billingInfo.address) &&
        $addressSelector.length &&
    ...
    extractShippingInfo($form) {
      ...
      $form
        .find(
          ':input[name^="shipping_information[shipping_profile][address][0][address]"]',
        )
        .each((index, element) => {
          // Extract the address field name.
          const $input = $(element);
          let name = $input.attr('name').split('[');
          name = name[name.length - 1];
          shippingInfo.address[name.substring(0, name.length - 1)] =
            $input.val();
        });

      const $addressSelector = $(
        'select[name="shipping_information[shipping_profile][select_address]"]',
        $form,
      );
      if (
        $.isEmptyObject(shippingInfo.address) &&
        $addressSelector.length &&
        $addressSelector.val() !== '_new'
      ) {
